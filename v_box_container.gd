...
@onready var cache:ItemList = $Cache
...
func add_cache_line(block:String="0", set:String="0", tag:String="0x0", offset:String="0x0", info:String="...") -> void:
	if(blockCount >= blockNumber): return
	cache.add_item(block)
	cache.add_item(set)
	cache.add_item(tag)
	cache.add_item(offset)
	cache.add_item(info)
	blockCount += 1
...
func modify_cache_line(index:int, block:String, set:String, tag:String, offset:String, info:String) -> void:
	...
	if offset != "keep":	cache.set_item_text(index+3, offset)
	if info != "keep": 	cache.set_item_text(index+4, info)
...
func get_cache_line(index:int) -> Dictionary:
	...
	return {	"block" : cache.get_item_text(index),
			...
			"offset" : cache.get_item_text(index+3),
			"info" : cache.get_item_text(index+4)}
